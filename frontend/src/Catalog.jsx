...
function StudentProposal({ challengeId, busy, setBusy }) {
  ...
  return <section className="panel"><h2>Готовы предложить решение?</h2><p className="intro">Расскажите о команде и подходе к задаче. Бизнес самостоятельно выберет предложение.</p>
    {!open ? <button className="primary" disabled={busy} onClick={() => setOpen(true)}>Предложить решение <span aria-hidden="true">→</span></button> : <form onSubmit={submit}>
      ...
      </label>)}</div><div className="actions"><button className="primary" disabled={busy || proposalFields.some(([key]) => key !== 'prototype_url' && !form[key].trim())}>{busy ? 'Отправляем…' : 'Отправить предложение'}</button><button type="button" className="text-button" disabled={busy} onClick={() => setOpen(false)}>Отмена</button></div>
    </form>}
  </section>
}
...
export default function Catalog({ mode, busy, setBusy, onManage }) {
  ...
  async function open(card) {
    ...
    try {
      ...
      if (ticket === selectionRequest.current) { setSelected(data); window.scrollTo({ top: 0, behavior: 'smooth' }) }
    } catch (err) { setError(err.message) }
  ...
  if (selected) return <section>
    ...
    <div className="detail-heading"><span className="industry-label">{selected.industry || 'Отрасль не указана'}</span><h1>{selected.title || 'Задача без названия'}</h1></div>
    <Notice>{error}</Notice><div className="workspace"><div className="main-column"><article className="panel"><h2>О задаче</h2><CardDetails card={selected} /></article>
      {mode === 'student' ? <StudentProposal key={selected.id} challengeId={selected.id} busy={busy} setBusy={setBusy} /> : <div className="panel"><h2>Работа с задачей</h2><p className="intro">Просматривайте заявки команд и принимайте решение в режиме Business.</p><button className="primary" disabled={busy} onClick={() => onManage(selected.id)}>Управлять задачей</button></div>}
    </div><ScoreCard card={selected} /></div>
  </section>
  ...
  return <section>
    ...
    <form className="filter-bar" onSubmit={(event) => { event.preventDefault(); setLoading(true); setQuery({ industry: filter.industry.trim(), readiness_level: filter.readiness_level }); setVersion((value) => value + 1) }}>
      <label><span>Отрасль</span><input value={filter.industry} onChange={(event) => setFilter({ ...filter, industry: event.target.value })} placeholder="Все отрасли" disabled={busy} /></label>
      <label><span>Готовность задачи</span><select value={filter.readiness_level} disabled={busy} onChange={(event) => setFilter({ ...filter, readiness_level: event.target.value })}><option value="">Все уровни</option>{levels.map((level) => <option key={level}>{level}</option>)}</select></label>
