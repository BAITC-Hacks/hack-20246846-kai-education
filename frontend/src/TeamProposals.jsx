...
export default function TeamProposals({ challengeId, active, busy, setBusy, user }) {
  ...
  async function decide(item, action) {
    if (user.verification_status !== 'verified') { setDecisionError('Для решения по предложению нужен подтверждённый бизнес-аккаунт.'); return }
  ...
  return <section className="panel team-section"><div className="section-heading"><h2 ref={headingRef} tabIndex={-1}>Предложения команд</h2><button className="text-button" disabled={busy || loading} onClick={() => { setLoading(true); setVersion((value) => value + 1) }}>Обновить</button></div>
    ...
    {user.verification_status !== 'verified' && <p className="review-callout">Для принятия или отклонения предложения нужен подтверждённый бизнес-аккаунт. Запросите проверку в верхней части страницы.</p>}
    ...
    {loading ? <p role="status">Загружаем предложения…</p> : items.length === 0 ? <div className="empty-state"><h3>Пока нет предложений</h3><p>Команды смогут откликнуться на задачу в режиме «Студент».</p></div> : <div className="team-list">{items.map((item) => <article className="team-card" key={item.id}>
      ...
      {item.status === 'pending' && <div className="actions"><button className="primary" disabled={busy || hasAccepted || user.verification_status !== 'verified'} onClick={() => { setDecisionError(''); setDecision({ item, action: 'accept' }) }}>Принять</button><button className="danger-button" disabled={busy || user.verification_status !== 'verified'} onClick={() => { setDecisionError(''); setDecision({ item, action: 'reject' }) }}>Отклонить</button></div>}
    </article>)}</div>}
